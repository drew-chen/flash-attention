...
torch::Tensor transpose_cuda(const torch::Tensor &input) {
    ...
    auto output = torch::empty({input.size(0), input.size(1), input.size(3), input.size(2)},
                               input.options());
...
torch::Tensor scale_cuda(const torch::Tensor &input, double factor) {
    ...
    scale_cuda_launch(output.mutable_data_ptr<float>(), batch_size, num_heads, input_width, input_height,
...
torch::Tensor softmax_cuda(const torch::Tensor &input) {
    ...
    softmax_test_cuda_launch(input.const_data_ptr<float>(), output.mutable_data_ptr<float>(), batch_size,
...
torch::Tensor matmul_cuda(const torch::Tensor &left, const torch::Tensor &right) {
    ...
    auto output = torch::zeros({left.size(0), left.size(1), left.size(2), right.size(3)},
                               left.options());
    matmul_cuda_launch(left.const_data_ptr<float>(), right.const_data_ptr<float>(),
                       output.mutable_data_ptr<float>(), batch_size, num_heads, left_height, shared_dim,
                       right_width);
    return output;
}
