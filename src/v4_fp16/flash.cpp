...
void flash_forward_v4_fp16_cuda_launch(const c10::Half *q,
                                        const c10::Half *k,
                                        const c10::Half *v,
                                        c10::Half *out,
                                        int batch_size,
                                        ...
                                        int kv_seq_len,
                                        int head_dim);
torch::Tensor flash_forward_v4_fp16_pytorch_cuda(const torch::Tensor &q,
                                                  const torch::Tensor &k,
                                                  const torch::Tensor &v);

void check_cuda_float16_contiguous_dim(const torch::Tensor &tensor,
                                       const char *name,
                                       int dim) {
...
bool can_use_v4_fp16_vector_loads(const torch::Tensor &q,
                                  const torch::Tensor &k,
                                  const torch::Tensor &v) {
    return is_vector_load_aligned(q) && is_vector_load_aligned(k) && is_vector_load_aligned(v);
}

void check_v4_fp16_kernel_requirements(const torch::Tensor &q,
                                        const torch::Tensor &k,
                                        const torch::Tensor &v) {
    TORCH_CHECK(q.size(3) == 64, "V4 FP16 supports only head dimension D=64");
    TORCH_CHECK(can_use_v4_fp16_vector_loads(q, k, v),
                "V4 FP16 requires q, k, and v base addresses to be 16-byte aligned");
}

torch::Tensor flash_forward_v4_fp16(torch::Tensor q, torch::Tensor k, torch::Tensor v) {
    validate_attention_inputs_float16(q, k, v);
    check_v4_fp16_kernel_requirements(q, k, v);
    return flash_forward_v4_fp16_pytorch_cuda(q, k, v);
}

torch::Tensor flash_forward_v4_fp16_unchecked(torch::Tensor q, torch::Tensor k, torch::Tensor v) {
    check_v4_fp16_kernel_requirements(q, k, v);
    return flash_forward_v4_fp16_pytorch_cuda(q, k, v);
}
...
torch::Tensor flash_forward_v4_fp16_pytorch_cuda(const torch::Tensor &q,
    ...
    flash_forward_v4_fp16_cuda_launch(
        q.const_data_ptr<c10::Half>(), k.const_data_ptr<c10::Half>(),
        v.const_data_ptr<c10::Half>(), out.mutable_data_ptr<c10::Half>(), batch_size, num_heads,
        query_seq_len, kv_seq_len, head_dim);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &flash_attention::flash_forward_v4_fp16,
          "V4 FP16 FlashAttention forward. Expects q [B, H, M, D] and k/v [B, H, N, D] CUDA "
          "float16 contiguous tensors with D=64 and 16-byte-aligned base addresses.");
    m.def("forward_unchecked", &flash_attention::flash_forward_v4_fp16_unchecked,
          "V4 FP16 FlashAttention forward without input validation. "
