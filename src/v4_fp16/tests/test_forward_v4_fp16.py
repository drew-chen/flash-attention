...
def test_forward_v4_fp16_matches_pytorch_reference(
    batch_size, num_heads, query_seq_len, kv_seq_len, head_dim
):
    """V4 FP16 contract: q may have M rows while k/v have N rows."""
    ...
    actual = flash_attention_v4_fp16.forward(q, k, v)

    assert actual.dtype == torch.float16
    torch.testing.assert_close(actual, expected, rtol=2e-2, atol=2e-2)


def test_forward_v4_fp16_rejects_float32_inputs():
    if not torch.cuda.is_available():
        pytest.skip("CUDA is not available")
    ...
    with pytest.raises(RuntimeError, match="q must be float16"):
        flash_attention_v4_fp16.forward(q, k, v)


def test_forward_v4_fp16_rejects_unsupported_head_dimension():
    if not torch.cuda.is_available():
        pytest.skip("CUDA is not available")
...
def test_forward_v4_fp16_rejects_misaligned_contiguous_inputs():
    if not torch.cuda.is_available():
        pytest.skip("CUDA is not available")
