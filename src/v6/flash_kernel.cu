...
/*
Optimizes V5's scalar overhead with __shfl_xor_sync and __expf while retaining
the synchronous K/V staging schedule.

flash_forward_v6_cuda_launch expects raw pointers for tensors shaped as:
...
static_assert(HEAD_DIM == 64, "V6 is specialized for D=64");
static_assert(THREAD_BLOCK_SZ % WARP_SIZE == 0,
              "V6 thread block size must contain a whole number of warps");
static_assert(B_r % NUM_WARPS == 0, "V6 Q rows must divide evenly among warps");
...
// Before launch, check_v6_kernel_requirements() in flash.cpp verifies that the
...
__device__ void load_shared_tile_vectorized(half *const smem_ptr,
    ...
    static_assert(HEAD_DIM == 64, "V6 vectorized tile loads assume D=64");
    ...
    for (int vector_i = static_cast<int>(threadIdx.x); vector_i < vectors_per_tile;
        ...
        // NVCC combines this fixed aligned sequence into STS.128. An explicit
        // Half8 pointer assignment was benchmarked and generated the same code.
#pragma unroll
...
/** Returns the shared buffer that K and V use during their disjoint lifetimes. */
__device__ __forceinline__ half *get_KV_buffer(half *const smem_ptr) {
    return smem_ptr + TileParams::sKV_shared_offset;
}

/** Loads K synchronously into the shared K/V buffer. */
__device__ half *load_K_j(const FlashForwardKernelParams &p,
                          half *const smem_ptr,
                          const int tile_j) {
    half *const sK_j = get_KV_buffer(smem_ptr);
    load_shared_tile_vectorized(sK_j, p.gK, p.H, p.N, B_c, tile_j, K_SHARED_STRIDE);
    return sK_j;
}

/** Loads V synchronously after K's aliased shared-memory lifetime ends. */
__device__ half *load_V_j(const FlashForwardKernelParams &p,
                          half *const smem_ptr,
                          const int tile_j) {
    half *const sV_j = get_KV_buffer(smem_ptr);
    load_shared_tile_vectorized(sV_j, p.gV, p.H, p.N, B_c, tile_j, K_SHARED_STRIDE);
    return sV_j;
}

/**
 * Reduces across a warp and returns the result in every lane.
 */
template <ReductionOp Op> __device__ __forceinline__ float warp_allreduce(float initial_value) {
    // XOR reduction leaves the complete result in every
    // lane after five shuffles. V5's down-reduction needs a sixth shuffle to
    // broadcast lane 0's result.
    float value = initial_value;
    for (int lane_mask = 16; lane_mask >= 1; lane_mask /= 2) {
        const float other = __shfl_xor_sync(FULL_WARP_MASK, value, lane_mask);
 ...
 *                      ([8, 64] @ [64, 32]  = [8, 32])
 *
 * Performs the matmul with WMMA into sS_ij, then returns this lane's eight
 * scores so softmax no longer depends on the shared result buffer.
 */
__device__ __forceinline__ std::array<float, WARP_TILE_ROWS> score_ij(const half *const sQ_i,
    ...
    for (int d = 0; d < HEAD_DIM; d += WMMA_K) {
        wmma::load_matrix_sync(a_frag,
                               sQ_i + (static_cast<ptrdiff_t>(row_start * HEAD_DIM)) + d, lda);
...
__device__ __forceinline__ void online_softmax_ij(half *const sP_ij_unnormalized,
    ...
    for (int warp_row = 0; warp_row < WARP_TILE_ROWS; ++warp_row) {
        ...
        const float m_old = wM_i_replicated[warp_row];
        ...
        const float m_new = query_valid ? fmaxf(m_old, key_tile_row_max) : 0.0F;
        // Use CUDA's fast approximate exponential. Softmax
        // dominates the scalar portion of this WMMA kernel, and measured FP16
        // output accuracy remains equivalent to the expf version.
        const float p_value = query_valid && key_valid ? __expf(score - m_new) : 0.0F;
        const float key_tile_row_sum = warp_allreduce<ReductionOp::SUM>(p_value);
        // Computing this once in lane 0 and
        // broadcasting it was slower, so retain the lane-local calculation.
        const float old_scale = query_valid ? __expf(m_old - m_new) : 0.0F;
...
__device__ __forceinline__ void accumulate_PV_into_wO_i(const half *const sP_ij_unnormalized,
    ...
    for (int output_col = 0; output_col < HEAD_DIM; output_col += WMMA_N) {
        ...
        for (int key_col = 0; key_col < B_c; key_col += WMMA_K) {
            const half *const p_fragment =
                sP_ij_unnormalized +
                (static_cast<std::size_t>(row_start) * P_SHARED_STRIDE) + key_col;
            wmma::load_matrix_sync(p_frag, p_fragment, ldp);
            wmma::load_matrix_sync(v_frag,
                                   sV_j + (key_col * K_SHARED_STRIDE) + output_col, ldv);
        ...
        for (int warp_row = 0; warp_row < WARP_TILE_ROWS; ++warp_row) {
            const float contribution =
                warp_sPV_ij[(warp_row * WMMA_RESULT_STRIDE) + lane];
            if (output_col == 0) {
 ...
 * accumulators stay in registers. Shared memory contains separate Q, P, and
 * WMMA-result buffers. K and V share storage because their lifetimes do not
 * overlap.
 */
...
#pragma unroll
    for (int warp_row = 0; warp_row < WARP_TILE_ROWS; ++warp_row) {
        wM_i_replicated[warp_row] = -std::numeric_limits<float>::infinity();
        wL_i_replicated[warp_row] = 0.0F;
    }
    ...
    for (int key_tile = 0; key_tile < key_tiles; ++key_tile) {
        half *const sKV_j = load_K_j(p, smem, key_tile);
        __syncthreads();

        const bool key_valid = (key_tile * B_c) + lane < p.N;
        // Keep this lane's eight scores in registers across softmax and V staging.
        auto wS_ij = score_ij(sQ_i, sKV_j, sS_ij, row_start, lane);
        ...
        // Finish reading K before V synchronously replaces it.
        __syncthreads();
        load_V_j(p, smem, key_tile);
        __syncthreads();

        accumulate_PV_into_wO_i(sP_ij_unnormalized, sKV_j, sS_ij, row_start, lane,
...
void launch_forward_d64(const c10::Half *const gQ,
                        ...
                        int N,
                        int D) {
    TORCH_CHECK(D == static_cast<int>(HEAD_DIM), "V6 CUDA kernel supports only D=64");
    ...
    // 27 KiB mixed-precision tile allocation.
...
/** Launches V6 with synchronous V staging, XOR reductions, and fast exponential. */
void flash_forward_v6_cuda_launch(const c10::Half *const gQ,
                                  const c10::Half *const gK,
                                  const c10::Half *const gV,
                                  c10::Half *const gO,
                                  int B,
                                  ...
                                  int N,
                                  int D) {
    launch_forward_d64(gQ, gK, gV, gO, B, H, M, N, D);
}
