...
void flash_forward_v6_cuda_launch(const c10::Half *q,
                                  const c10::Half *k,
                                  const c10::Half *v,
                                  c10::Half *out,
                                  int batch_size,
                                  ...
                                  int kv_seq_len,
                                  int head_dim);
torch::Tensor flash_forward_v6_pytorch_cuda(const torch::Tensor &q,
...
bool can_use_v6_vector_loads(const torch::Tensor &q,
                             const torch::Tensor &k,
                             const torch::Tensor &v) {
    return is_vector_load_aligned(q) && is_vector_load_aligned(k) && is_vector_load_aligned(v);
}

void check_v6_kernel_requirements(const torch::Tensor &q,
                                  const torch::Tensor &k,
                                  const torch::Tensor &v) {
    TORCH_CHECK(q.size(3) == 64, "V6 supports only head dimension D=64");
    TORCH_CHECK(can_use_v6_vector_loads(q, k, v),
                "V6 requires q, k, and v base addresses to be 16-byte aligned");
}

torch::Tensor flash_forward_v6(torch::Tensor q, torch::Tensor k, torch::Tensor v) {
    validate_attention_inputs_float16(q, k, v);
    check_v6_kernel_requirements(q, k, v);
    return flash_forward_v6_pytorch_cuda(q, k, v);
}

torch::Tensor flash_forward_v6_unchecked(torch::Tensor q,
                                         torch::Tensor k,
                                         torch::Tensor v) {
    check_v6_kernel_requirements(q, k, v);
    return flash_forward_v6_pytorch_cuda(q, k, v);
...
torch::Tensor flash_forward_v6_pytorch_cuda(const torch::Tensor &q,
    ...
    flash_forward_v6_cuda_launch(q.const_data_ptr<c10::Half>(), k.const_data_ptr<c10::Half>(),
                                 v.const_data_ptr<c10::Half>(),
                                 out.mutable_data_ptr<c10::Half>(), batch_size, num_heads,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &flash_attention::flash_forward_v6,
          "V6 FlashAttention forward. Expects q [B, H, M, D] and k/v [B, H, N, D] CUDA "
          "float16 contiguous tensors with D=64 and 16-byte-aligned base addresses.");
    m.def("forward_unchecked", &flash_attention::flash_forward_v6_unchecked,
          "V6 FlashAttention forward without input validation. "
