...
struct ReductionPolicy {    
    ...
    static __device__ __forceinline__ T identity() {
        if constexpr (Op == ReductionOp::SUM) return static_cast<T>(0);
        if constexpr (Op == ReductionOp::MAX) return static_cast<T>(-1e38); // simplified -inf
    }
    ...
    static __device__ __forceinline__ void apply_atomic(T* result, const T val) {
        if constexpr (Op == ReductionOp::SUM) atomicAdd(result, val);
        else if constexpr (Op == ReductionOp::MAX) atomicMaxFloat(result, val);
    }
...
private: 
    static __device__ __forceinline__ T reduce(const T accumulator, const T val) {
        if constexpr (Op == ReductionOp::SUM) return gpu_add(accumulator, val);
        if constexpr (Op == ReductionOp::MAX) return gpu_max(accumulator, val);
    }
};
...
struct MapExpMinus {
    ...
    __device__ __forceinline__ T operator()(const T& val) const {
        ...
        const int stat_offset = batch_head_offset(batch_idx, head_idx, num_heads, rows) + blockIdx.y;
...
template <typename T, ReductionOp Op, unsigned int BlockSize, int CoarsenFactor, Mapper<T> M = MapIdentity<T>>
__global__ void map_reduce_kernel(const T* __restrict__ input, 
                            ...
                            // this is a compiler hint that this will never chang ethroughout lifetime of program
                            // __grid_constant__ ensures all threads use the same single address in constant memory,
                            // saving significant memory and improving performance.
    ...
    /*
    Note: N <= 500,000, so we do not need to do a grid stride since max grid size is 2^31 - 1. We can assume 
    # elements processed = block # of threads * coarsen
    To coarsen, we have to repeat the gathering of input elements by the # of times we coarsen
    To get our block to read independent elements, we conceptually divide the input into multiple blocks of elements
    per block of threads.
    So with coarsen = 4, we iterate over 4 blocks of data with our 1 block of threads
    */
    #pragma unroll
...
__global__ void map_kernel(const T* input, T* __restrict__ output, const int num_heads,
                           const int rows, const int head_dim, const __grid_constant__ M mapper) {
...
inline void softmax_rows_launch(const float* input, float* output, int batch_size, int num_heads,
    ...
    map_reduce_kernel<float, ReductionOp::SUM, mr_block_size, coarsen, MapExpMinus<float>>
        <<<reduce_grid, mr_block_size>>>(input, output, device_exp_sum_ptr, num_heads, rows, head_dim,
    ...
    map_kernel<<<map_grid, map_block_size>>>(output, output, num_heads, rows, head_dim,
                                             MapDivision<float>{device_exp_sum_ptr, num_heads, rows});
